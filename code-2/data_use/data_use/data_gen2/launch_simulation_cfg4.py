...
def initialConditions(n:int = None)->tuple:
    ...
    elif INIT_SIM == 'smallest':
        lim = 0.85 * 10

        xPos = np.array([-4, 4])
        yPos = np.array([0])
        gridX, gridY = np.meshgrid(xPos, yPos)
        delta = np.random.uniform(0, 6, gridX.shape + (2,))
...
def runSim(pathTorch:str, pathSim:str, params:SimulationParameters, cond:tuple, nbHist:int = NB_HIST, saveBool:bool = False)-> tuple:
    ...
    nodesFeatures, yVect, edgeFeaturesVect, edgeIndexVect = ft.processSimulation(resOutput, dt = DT_PARAM, nbhist = nbHist)

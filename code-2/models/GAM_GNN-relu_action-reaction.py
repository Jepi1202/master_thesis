...
class GAM_GNN(nn.Module):
    ...
    def L1Reg(self, graph):
        ...
        weights = self.GNN.relu_layer(weights)

        loss = self.scaler_regu * torch.sum(torch.abs(weights)) / graph.edge_index[0, :].shape[0]
    ...
    def get_weights(self, graph):
        ...
        weights = self.GNN.relu_layer(weights.reshape(prods.shape)).reshape(w_shape)

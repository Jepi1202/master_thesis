...
def compute_interaction(attr, k = 70, epsilon = 0.5):
    """
    Compute the force ...
    ...
    force = torch.zeros((attr.shape[0], 2)).to(device)

    rij = torch.zeros((attr.shape[0], 2)).to(device)
    rij[:, 0] = attr[:, 0] * attr[:, 1]
    rij[:, 1] = attr[:, 0] * attr[:, 2]
...
class GN_edge(MessagePassing):
    ...
    def __init__(self, d, aggr:str='add'):
        ...
        self.messageShape = d['message_shape']
        
        
        self.norm = torch.nn.LayerNorm(self.messageShape+ self.inputShape)
        
        self.updateMLP = MLP(inputShape = self.messageShape+ self.inputShape, 
    ...
    def message(self, x_i:torch.tensor, x_j:torch.tensor, edge_attr: torch.tensor):
        ...
        y = compute_interaction(edge_attr, k = 70, epsilon = 0.5)
        return y

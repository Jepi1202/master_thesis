...
print(">>>>>>>>>>>> LOADOING THE V2 OF FEATURES")
...
DT = cfg['simulation']['parameters']['dt']
...
def predictSpeeds(mat:np.array, dt)->list:
    """
...
def getSpeeds(mat:np.array, dt)->np.array:
    """ 
    ...
    # ok for predictions
    return (mat[inds, :, :] - mat[inds0, :, :]) / dt



def getSpeedFeatures(mat: np.array, nb: int, dt)->np.array:
    """ 
...
def getFeatures(mat: np.array, dt, nb:int = NB_HIST)->tuple:
    """ 
...
def processSimulation(sim, dt= DT, radius = None, nbhist = NB_HIST):
    
    if radius is None:
        radius = np.ones(sim.shape[1]) * R_PARAM
    
    x,y = getFeatures(sim.copy(), dt, nb=nbhist)

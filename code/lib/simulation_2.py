...
def compute_main(N, params, boundary, T = 100, dt = 0.01, seed = 20, cutoff = 6,radii = 1.0, noiseBool = False, debug=False, initialization = None):
    ...
    for t in tqdm(range(T)):
        neighbours = find_neighbours(positions, cutoff = cutoff)
        ...
        for i in range(N):
            ...
            corrected_force, cosAndSin[i, :] = boundary_effects(positions[i], total_force, cosAndSin=cosAndSin[i, :], boundary=boundary)

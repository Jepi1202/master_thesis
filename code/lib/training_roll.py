...
def dataAugFun(data, device = DEVICE):
    """ 
    ...
    p2 = np.random.random()
    if p2 < 0.8:   #noise
        rdNb = torch.normal(mean=0, std=0.003, size=data.x.shape).to(device)
        rdNb2 = torch.normal(mean=0, std=0.01, size=data.edge_attr.shape).to(device)
        #rdNb3 = torch.normal(mean=0, std=0.01, size=data[:, 1:].edge_attr.shape).to(device)
...
def lossFun(preds:torch.tensor, gts:torch.tensor, binaries:Optional[Union[None, torch.tensor]] = None, topk = None, gamma = True):
    """ 
    ...
    if gamma:
        # turn into float ?
        g = torch.arange(res.shape[0]).unsqueeze(-1).unsqueeze(-1)
        res = res * g
...
def train(model, optimizer, lr_scheduler,  paramLearning, path_result_LAST, path_result_MIN, device = DEVICE):
    ...
    for epoch in range(nbEpoch):
        ...
        for data, idx in tqdm(loader):
            ...
            nb_rolling = np.random.randint(1, NB_ROLL+1)
            
            bina = (torch.abs(data.x[:, 0]) <= LIMIT_DIST) & (torch.abs(data.x[:, 1]) <= LIMIT_DIST)
            bina = bina.repeat(2*nb_rolling, 1).swapaxes(0,1).to(DEVICE)
            ...
            pos = data.x[:, :2].clone()    # keep the comp graph ?: adds an identity actually
            ...
            #loss0 = 100* lossFun(out, y[:, :(2 * nb_rolling)], bina, topk = int(0.1 * 2 * nb_rolling * y.shape[0]))
            loss0 = 100* lossFun(out, y[:, :(2 * nb_rolling)], bina, topk = None)
            ...
            if ((i+1) % 500 == 0 or i == 0):
                ...
                with torch.no_grad():
                    ...
                    pathSim = '/scratch/users/jpierre/test_speed_50_80/test/np_file/output_0.npy'
...
if __name__ == '__main__':
    ...
    path_result_MIN = os.path.join(result_model_path, f'{name}_comp-n-speed_min.pt')
    path_result_LAST = os.path.join(result_model_path, f'{name}_comp-n-speed_last.pt')

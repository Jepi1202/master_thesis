...
def generate_sim(model, 
    ...
    ## rollout simulation
    with torch.no_grad():
        out = genSim(model, nbStep, init_state, pos, radius=None, train = False, debug = None, dt_scale = dt_scale)
...
def generate_sim_batch(model, 
    ...
    for i in range(gt.shape[0]):
        res[i] = generate_sim(model = model, 
                              gt = gt[i], 
                              initId = initId, 
                              nbStep = nbStep, 
                              device = device,
                              dt_scale = dt_scale).detach().cpu().numpy()

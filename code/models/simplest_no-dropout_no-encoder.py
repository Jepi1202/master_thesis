...
class Simplest(nn.Module):
    ...
    def sum_message(self, graph):
        ...
        aggr_messages = scatter(messages, edge_index[1, :], dim = 0,dim_size=x.size(0), reduce = 'sum')

        xVal = self.GNN.norm(torch.cat([x, aggr_messages], dim=-1))

...
def train(model, optimizer, lr_scheduler,  paramLearning, path_result_LAST, path_result_MIN, device = DEVICE):
    ...
    for epoch in range(nbEpoch):
        ...
        for data, idx in tqdm(loader):
            ...
            #pos = data.x[:, :2].clone()    # keep the comp graph: add a identity actually?
                                
            out = model(data)
            j = j+1

            loss0 = 100* lossFun(out, y[:, :2], bina, topk = None)
...
if __name__ == '__main__':
    ...
    loader = getLoader(p_train, batch_size = 128)

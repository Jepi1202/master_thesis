...
PATH = ['/master/code/results/models/normal/0-001']
...
DISPLAY_PATH = ['/master/code/results/1-step/normal_0-001']
...
def main():
    ...
    for i in range(len(list_exp)):
        ...
        for j in range(models):
            ...
            plotDegreeLoss(deg_list, errors.tolist())
            plt.savefig(os.path.join(path, 'errors-degs.png'))
            plt.close()
            ...
            plt.savefig(os.path.join(path, 'mae-speed-norm-scatter.png'))
            plt.close()
            ...
            plt.savefig(os.path.join(path, 'mae-speed-norm-boxplots.png'))
            plt.close()

...
def main(device = DEVICE):
    ...
    params.nbRoll = trainingInfos['nbRoll']
    params.batchRollout = trainingInfos['batchRollout']
    ...
    p_sim = os.path.join(p_data, 'training/torch_file')
    p_sim = os.path.join(p_data, 'validation/np_file')
    p_sim_training = os.path.join(p_data, 'training/np_file')
    ...
    if pathJsonBool:
        ...
        loaderEval = getLoader(p_data, batch_size = batchSizeEval, mode = 'validation')
    ...
    loaderSimTraining = dl.simLoader2(p_sim_training)
    loaderSimTraining = DataLoaderPy(loaderSimTraining, batch_size=1, shuffle=False)
    
    
    params.loader = loaderSimTraining
    ...
    model = tr.train_rollout(model, params, device = device)
